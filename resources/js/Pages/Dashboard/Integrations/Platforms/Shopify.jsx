...
export default function Shopify({ connection }) {
        ...
        <SaasLayout pageHeader={{
            ...
            {connected && (
                <div className="mb-4 px-4 py-3 rounded-lg bg-emerald-500/10 border border-emerald-500/30 text-emerald-200 text-sm flex items-center gap-2">
                    <CheckCircle2 className="w-4 h-4" /> Connected to <strong>{connection.shop_domain}</strong>
                </div>
            )}

            <form onSubmit={submit} className="bg-surface-2 border border-line rounded-xl p-6 max-w-2xl space-y-5">
                <p className="text-xs text-content-muted">
                    Create a private app in <strong>Settings → Apps → Develop apps</strong> and copy the Admin API access token.
                </p>
                ...
                <Field label="Access Token" type="password" value={data.access_token} onChange={(v) => setData('access_token', v)} error={errors.access_token} placeholder="shpat_…" required />
