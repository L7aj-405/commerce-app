    ...
    return (
        <SaasLayout pageHeader={{
            ...
            <div className="max-w-2xl space-y-6">
                {connection && (
                    <div className={`px-4 py-3 rounded-[var(--radius-button)] border text-sm flex items-center gap-2 ${
...
function MethodCard({ icon: Icon, title, badge, badgeTone, description, active, disabled, onClick }) {
    ...
    return (
        <button
            ...
            disabled={disabled}
            onClick={onClick}
            className={`text-left p-4 rounded-[var(--radius-card)] border-2 transition ${
                disabled
        ...
        >
            <Icon className={`w-5 h-5 mb-2 ${active ? 'text-primary' : 'text-content-muted'}`} />
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-3 py-2 rounded-[var(--radius-button)] bg-surface-3 border ${errors.client_secret ? 'border-danger' : 'border-line'} text-content focus:outline-none focus:ring-2 focus:ring-primary`}
                        />
                        {errors.client_secret && <p className="mt-1 text-xs text-danger">{errors.client_secret}</p>}
...
const DIAGNOSTICS_STATUS_META = {
    connected: { label: 'Connected', badgeStatus: 'synced', tone: 'text-success' },
    partially_configured: { label: 'Partially configured', badgeStatus: 'pending', tone: 'text-warning' },
    failed: { label: 'Failed', badgeStatus: 'failed', tone: 'text-danger' },
        ...
        <Card title="Capability diagnostics">
            ...
            <div className="flex items-center justify-between flex-wrap gap-2 mb-3">
                ...
                <button
                    ...
                    disabled={running}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-[var(--radius-button)] bg-surface-3 border border-line text-content hover:bg-content/10 disabled:opacity-50"
                >
            ...
            {diagnostics?.capabilities?.length > 0 ? (
                <div className="border border-line rounded-[var(--radius-button)] overflow-hidden overflow-x-auto">
    ...
    return (
        <div className="space-y-6">
                ...
                <form onSubmit={submit} className="space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            className={`w-full px-3 py-2 rounded-[var(--radius-button)] bg-surface-3 border ${errors.webhook_secret ? 'border-danger' : 'border-line'} text-content focus:outline-none focus:ring-2 focus:ring-primary`}
                        />
                        {errors.webhook_secret && <p className="mt-1 text-xs text-danger">{errors.webhook_secret}</p>}
                    </div>
                    ...
                    <div>
                        ...
                        <div className="grid grid-cols-2 gap-2">
                            {webhookEvents.map((key) => (
                                <label key={key} className="flex items-center gap-2 px-3 py-2 rounded-[var(--radius-button)] bg-surface-3 border border-line text-sm text-content cursor-pointer">
            ...
            <Card title="Webhook URL" subtitle="Paste this into every webhook you add in Shopify">
                {webhookUrl ? (
                    <div className="flex items-center gap-2">
                        <input readOnly value={webhookUrl} className="flex-1 px-3 py-2 rounded-[var(--radius-button)] bg-surface-3 border border-line text-content text-sm font-mono" />
                        <button type="button" onClick={copyUrl} className="inline-flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-[var(--radius-button)] bg-surface-3 border border-line text-content hover:bg-content/10">
