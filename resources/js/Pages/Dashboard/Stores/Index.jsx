...
export default function Index({ stores = [], currentStoreId }) {
    ...
    return (
        <SaasLayout pageHeader={{
            ...
            {stores.length === 0 ? (
                <div className="bg-surface-2 border border-line rounded-[var(--radius-card)]">
                    <EmptyState
                        icon={StoreIcon}
                        title="You don't have any stores yet"
                        description="Create your first store to start ringing up sales."
                        action={
                            <Link href="/dashboard/stores/create" className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-[var(--radius-button)] bg-primary text-primary-contrast hover:bg-primary-strong">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    {stores.map((s) => {
                        ...
                        return (
                            <div
                                key={s.id}
                                className={`bg-surface-2 border rounded-[var(--radius-card)] p-5 transition ${
                                    isCurrent ? 'border-primary/40 ring-1 ring-primary/20' : 'border-line hover:border-line'
                                }`}
                            >
                                <div className="flex items-start justify-between gap-3 mb-3">
                                    <div className="w-10 h-10 rounded-[var(--radius-button)] bg-gradient-to-br from-primary to-primary-strong text-primary-contrast font-bold flex items-center justify-center text-sm">
                                        {(s.name ?? '?').slice(0, 1).toUpperCase()}
                                    </div>
                                    {isCurrent && (
                                        <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider rounded-full bg-primary-soft text-primary">
                                ...
                                <div className="mt-4 flex items-center gap-2">
                                    {! isCurrent && (
                                        <button
                                            ...
                                            className="flex-1 inline-flex items-center justify-center gap-1.5 px-3 py-2 text-xs font-semibold rounded-[var(--radius-button)] bg-primary text-primary-contrast hover:bg-primary-strong transition"
                                        >
                                    ...
                                    <Link
                                        ...
                                        className="inline-flex items-center gap-1 px-2.5 py-2 text-xs rounded-[var(--radius-button)] border border-line text-content-muted hover:bg-surface-3 hover:text-content"
                                        title="Settings"
                                    >
