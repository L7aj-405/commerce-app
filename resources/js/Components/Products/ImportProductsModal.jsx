...
function Option({ icon: Icon, title, subtitle, disabled }) {
    return (
        <div className={`flex items-center gap-3 p-3 rounded-lg border border-line ${disabled ? 'opacity-50' : ''}`}>
...
function OptionButton({ icon: Icon, title, subtitle, onClick }) {
    return (
        <button type="button" onClick={onClick} className="w-full flex items-center gap-3 p-3 rounded-lg border border-line bg-surface hover:bg-surface-3 text-left transition">
            <Icon className="w-4 h-4 text-content-muted flex-shrink-0" />
            <div className="min-w-0">
                <p className="text-sm font-medium text-content">{title}</p>
                <p className="text-xs text-content-muted">{subtitle}</p>
...
function OptionLink({ icon: Icon, title, subtitle, href }) {
    return (
        <Link href={href} className="w-full flex items-center gap-3 p-3 rounded-lg border border-line bg-surface hover:bg-surface-3 text-left transition">
            <Icon className="w-4 h-4 text-content-muted flex-shrink-0" />
            <div className="min-w-0">
                <p className="text-sm font-medium text-content">{title}</p>
                <p className="text-xs text-content-muted">{subtitle}</p>
