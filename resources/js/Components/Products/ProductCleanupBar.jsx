...
export default function ProductCleanupBar({ selectedIds, connections = [], onDone, onClear }) {
    ...
    return (
        <>
            <button type="button" onClick={() => setModal('archive')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-[var(--radius-button)] bg-surface-2 border border-line text-content hover:bg-surface-3">
                <Archive className="w-3.5 h-3.5" /> Archive
            </button>
            <button type="button" onClick={() => setModal('unlink')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-[var(--radius-button)] bg-surface-2 border border-line text-content hover:bg-surface-3">
                <Unlink className="w-3.5 h-3.5" /> Unlink from channel
            </button>
            <button type="button" onClick={() => setModal('reset-sync')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-[var(--radius-button)] bg-surface-2 border border-line text-content hover:bg-surface-3">
                <RotateCcw className="w-3.5 h-3.5" /> Reset sync
            </button>
            <button type="button" onClick={() => setModal('purge')} className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-[var(--radius-button)] bg-danger text-white hover:brightness-90">
...
function ModalShell({ title, subtitle, onClose, children, footer, danger = false }) {
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-surface-2 border border-line rounded-[var(--radius-card)] shadow-xl max-w-lg w-full max-h-[85vh] overflow-hidden flex flex-col text-left">
                <div className={`p-4 border-b border-line flex justify-between items-center ${danger ? 'bg-danger-soft' : 'bg-surface'}`}>
                    <div>
        ...
        <ModalShell
            ...
            footer={(
                <>
                    ...
                    <button type="button" onClick={submit} disabled={busy} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-[var(--radius-button)] bg-primary text-primary-contrast hover:bg-primary-strong disabled:opacity-50">
    ...
    return (
        <ModalShell
            ...
            footer={(
                <>
                    <button type="button" onClick={onClose} className="px-3 py-2 text-sm font-medium rounded-[var(--radius-button)] bg-surface-3 border border-line text-content hover:bg-content/10">Cancel</button>
                    <button type="button" onClick={submit} disabled={busy || ! connectionId} className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-semibold rounded-[var(--radius-button)] bg-primary text-primary-contrast hover:bg-primary-strong disabled:opacity-50">
        ...
        >
            ...
            <label className="block text-sm text-content-muted">
                ...
                <select
                    ...
                    className="mt-1 w-full px-3 py-2 text-sm rounded-[var(--radius-button)] bg-surface border border-line text-content"
                >
...
function PurgeModal({ count, productIds, onClose, onDone }) {
    ...
    return (
        <ModalShell
            ...
            danger
            footer={results ? (
                <button type="button" onClick={onDone} className="ml-auto px-4 py-2 text-sm font-medium rounded-[var(--radius-button)] bg-surface-3 border border-line text-content hover:bg-content/10">Close</button>
            ) : (
                <>
                    <button type="button" onClick={onClose} className="px-3 py-2 text-sm font-medium rounded-[var(--radius-button)] bg-surface-3 border border-line text-content hover:bg-content/10">Cancel</button>
                    <button
                ...
                <>
                    <div className="grid grid-cols-2 gap-2 text-center">
                        <div className="p-2 rounded-[var(--radius-button)] bg-surface border border-line">
                            <div className="text-lg font-bold tabular-nums text-success">{allowed}</div>
                            <div className="text-[10px] uppercase text-content-muted">Allowed</div>
                        </div>
                        <div className="p-2 rounded-[var(--radius-button)] bg-surface border border-line">
                            <div className="text-lg font-bold tabular-nums text-danger">{blocked}</div>
...
function SkippedProductRow({ product, purged }) {
    ...
    return (
        <div className="p-2 rounded-[var(--radius-button)] border border-line bg-surface text-xs">
            <div className="flex items-start justify-between gap-2">
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                    {purged === true && <span className="text-success">Purged</span>}
                    {purged === null && <span className="text-success">purgeable</span>}
...
function SkippedFollowUpActions({ skippedIds, onDone }) {
    ...
    return (
        <div className="p-2.5 rounded-[var(--radius-button)] border border-warning/30 bg-warning-soft space-y-2">
            ...
            <div className="flex items-center gap-2 flex-wrap">
                <button
                    ...
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-[var(--radius-button)] bg-surface-2 border border-line text-content hover:bg-surface-3 disabled:opacity-50"
                >
