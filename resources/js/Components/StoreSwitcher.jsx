...
export default function StoreSwitcher({ collapsed = false, tone = 'indigo' }) {
    ...
    return (
        <div ref={ref} className="relative">
            ...
            >
                <div className={`flex-shrink-0 w-8 h-8 rounded-md bg-gradient-to-br ${colors.avatar} text-white text-xs font-bold flex items-center justify-center`}>
                            ...
                            <li key={s.id}>
                                <button
                                    ...
                                    className={`w-full flex items-center gap-2 px-2.5 py-2 text-sm transition ${
                                        isActive ? colors.active : 'text-content-muted hover:bg-surface-3'
                                    }`}
                                >
                                    <div className={`w-6 h-6 rounded-md bg-gradient-to-br ${colors.avatar} opacity-70 text-white text-[10px] font-bold flex items-center justify-center flex-shrink-0`}>
                                        {initials(s.name)}
                                    </div>
                                    ...
                                    {isSwitching ? (
                                        <span className={`w-3 h-3 rounded-full border-2 border-content-muted/40 ${colors.spinner} animate-spin`} />
                                    ) : isActive ? (
